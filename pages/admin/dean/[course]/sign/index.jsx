...
export default function DeanSign({ course, session, endpoint }) {
  socket = io(endpoint);
  ...
  const handleSign = async () => {
    try {
      const { data } = await axios.post("/api/admin/dean/sign", {
        studentid: selectedId,
  ...
  useEffect(() => {
    ...
    socket.emit(
      "clearance " + course.short.toLowerCase() + " initial",
      course.short
    );
    socket.on(
      "clearance " + course.short.toLowerCase() + " data initial",
      (students) => {
        setList(students);
      }
    );
    socket.on(
      "clearance " + course.short.toLowerCase() + " data update",
      (students) => {
        setList(students);
      }
    );
  ...
  return (
    <>
      ...
      <div className="hidden h-screen w-screen pt-20 lg:flex">
        <Head>
          <title>For Signing | {session.department} - Dean&apos;s Office</title>
        </Head>
        <TopBar />
        <div className="h-full w-1/4">
          <SideBarDean path="sign" session={session} course={course} />
        </div>
