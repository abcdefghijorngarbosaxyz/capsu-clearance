...
export default function CollectionSign({ session, period, endpoint }) {
  socket = io(endpoint);
  ...
  const handleSign = async () => {
    try {
      const { data } = await axios.post("/api/admin/collecting/sign", {
        studentid: selectedId,
  ...
  useEffect(() => {
    ...
    socket.emit("clearance collecting initial", course.short);
    socket.on("clearance collecting data initial", (students) => {
      setList(students);
    });
    socket.on("clearance collecting data update", (students) => {
      setList(students);
    });
  }, []);
...
export const getServerSideProps = async (context) => {
  ...
  if (session) {
    ...
    if (role === "Admin" && department === "Collecting") {
      return {
        props: {
